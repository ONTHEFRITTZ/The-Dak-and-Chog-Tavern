...
// Now: loads only on on-chain tables and renders bottom-right.

(function () {
  // Respect explicit table mode (set in table.html)
  const MODE = (document.documentElement.getAttribute('data-table-mode') || '').toLowerCase();
  if (MODE !== 'onchain') {
    // Don't render anything on free-to-play tables
    return;
  }
  ...
  function renderPanel() {
    ...
    if (!wrap) {
      wrap = el('div', 'aa-panel agent-panel');
      wrap.id = 'agent-panel';
      wrap.setAttribute('data-aa-panel', ''); // let CSS hide it if needed
      // Bottom-right, not buried by sidebar/top banner
      wrap.style.cssText = [
  ...
  window.addEventListener('poker:handstart', () => {
    ...
    const _ = loadCfg(); // ensure LS consistency
  ...
  try {
    window.AgentBudget = { isEnabled, canSpend, noteSpend, getConfig: loadCfg };
  } catch {}
})();
