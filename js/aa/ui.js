...
import { ensureMonadSelected, getAccounts } from './account.js';
import { MONAD } from './config.js';
import { AA, initAA } from '../aaClient.js';

function short(addr) { return addr && addr.length > 10 ? `${addr.slice(0, 6)}...${addr.slice(-4)}` : (addr || ''); }
...
function ensureContainer() {
  ...
  if (!c) {
    c = document.createElement('div'); c.id = 'aa-controls';
    c.style.cssText = ['display:flex','flex-direction:column','gap:6px','align-items:flex-start','margin-left:8px','max-width:360px','font-size:12px','color:#f5f5f5'].join(';');
    host.appendChild(c);
  } else if (host && c.parentElement !== host) { host.appendChild(c); }
  if (preferredHost) { c.style.display='flex'; c.style.marginLeft='0'; c.style.width='100%'; c.style.maxWidth='100%'; }
  else { c.style.display='none'; c.style.marginLeft='8px'; c.style.maxWidth='360px'; c.style.width=''; }
  return c;
}
...
function ensureStatusBox() {
  ...
  if (!box) { box = document.createElement('div'); box.className='aa-status'; box.style.cssText=['display:flex','flex-wrap:wrap','gap:6px','align-items:center','background:rgba(0,0,0,0.45)','padding:6px 10px','border-radius:10px','font-weight:600'].join(';'); c.appendChild(box); }
  return box;
}

function ensureActionsBox() { const c = ensureContainer(); let box = c.querySelector('#aa-actions'); if (!box) { box=document.createElement('div'); box.id='aa-actions'; box.style.cssText=['display:flex','flex-wrap:wrap','gap:6px','align-items:center'].join(';'); c.appendChild(box);} return box; }

function setBusy(button, busy, labelWhenBusy = 'Working...') { if (!button) return; if (busy) { if (!button.dataset.label) button.dataset.label = button.textContent || button.value || ''; button.disabled = true; button.textContent = labelWhenBusy; } else { const original = button.dataset.label || button.textContent; button.disabled = false; button.textContent = original; } }
...
async function hydrate() {
  if (__IS_F2P) return;
  let chainOk = false; try { chainOk = await ensureMonadSelected({ requestSwitch: false }); } catch {}
  const accounts = await getAccounts(); const addr = (accounts[0] || '').toLowerCase();
  const smartOptIn = (localStorage.getItem(SMART_ACCOUNT_OPT_IN_KEY) === 'true');
  let smartType = 'fallback'; try { if (smartOptIn) { await initAA({}); smartType = AA.smartAccountType || (AA.sponsored ? 'aa4337' : 'fallback'); } } catch {}
...
function renderButtons(state) {
  ...
  const actions = ensureActionsBox(); actions.innerHTML = '';
  ...
  const safeRun = (btn, fn, labelBusy) => { btn.addEventListener('click', async ()=>{ if(btn.disabled) return; setBusy(btn, true, labelBusy); try{ await fn(); } catch(e){ console.error('[aa/ui] action failed', e); alert(e?.message||'Operation failed.'); } finally { setBusy(btn, false); hydrate().catch(()=>{}); } }); };
