// Poker Lobby (Beta): filter tables with id starting 'poker-'
...
function renderLobby(list) {
  try {
    lobbyList.innerHTML = '';
    const onlyPoker = Array.isArray(list) ? list.filter(r => String(r.id||'').startsWith('poker-')) : [];
    const sorted = onlyPoker.sort((a,b)=>{
    ...
    sorted.forEach(row => {
      ...
      else { btn.textContent = 'Join'; btn.onclick = () => { window.location.href = `/games/poker/index.html?table=${encodeURIComponent(row.id)}`; }; }
      card.appendChild(left); card.appendChild(btn);
      lobbyList.appendChild(card);
    });
    if (!sorted.length) lobbyList.innerHTML = '<div style="opacity:.7; font-size:13px;">No poker tables yet. Please wait…</div>';
  } catch {}
}
