// js/aa/ui.js
// Simplified AA UI: Wallet | Gasless Mode | Network | Gas | SA(7702)
...
function ensureContainer() {
  ...
  if (!c) {
    ...
    c.style.cssText = [
      ...
      'margin-left:8px','max-width:360px','font-size:12px','color:#f5f5f5'
    ].join(';');
    host.appendChild(c);
  ...
  if (preferredHost) {
    c.style.display='flex'; c.style.marginLeft='0'; c.style.width='100%'; c.style.maxWidth='100%';
  } else {
    c.style.display='none'; c.style.marginLeft='8px'; c.style.maxWidth='360px'; c.style.width='';
  }
  return c;
}
...
function ensureStatusBox() {
  ...
  if (!box) {
    ...
    box.style.cssText = [
      ...
      'background:rgba(0,0,0,0.45)','padding:6px 10px','border-radius:10px','font-weight:600'
    ].join(';');
    c.appendChild(box);
  }
  return box;
...
function ensureActionsBox() {
  ...
  if (!box) {
    box = document.createElement('div');
    box.id = 'aa-actions';
    box.style.cssText = ['display:flex','flex-wrap:wrap','gap:6px','align-items:center'].join(';');
    c.appendChild(box);
  }
  return box;
}
...
function setBusy(button, busy, labelWhenBusy = 'Working...') {
  if (!button) return;
  if (busy) {
    if (!button.dataset.label) button.dataset.label = button.textContent || button.value || '';
...
function renderStatus(state) {
  const { controller, smartAddress, smartType, chainOk, sponsored } = state;
  const box = ensureStatusBox();
  const statusBits = [];
  statusBits.push(controller ? `Wallet: ${short(controller)}` : 'Wallet: not connected');
  statusBits.push(smartType === 'aa4337' ? `Gasless Mode: ${short(smartAddress)}` : 'Gasless Mode: fallback (EOA)');
  ...
  let sa7702 = 'not-ready';
  try { sa7702 = (localStorage.getItem('aa.7702.ready') === 'true') ? 'ready' : 'not-ready'; } catch {}
  statusBits.push(`SA(7702): ${sa7702}`);
  box.textContent = statusBits.join(' | ');
  try {
    let tip = box.querySelector('.aa-tip-7702');
    if (!tip) {
      tip = document.createElement('span');
      tip.className = 'aa-tip-7702';
      tip.style.cssText = 'margin-left:6px;opacity:.9;cursor:help';
      tip.textContent = 'i';
      box.appendChild(tip);
    }
    tip.title = 'When SA(7702) is ready, we switch to internal-signer delegation (derive SA -> minimal open delegation -> mmAccount.signDelegation). No extra clicks; no re-sign loops.';
  } catch {}
}
...
async function hydrate() {
  ...
  try {
    if (smartOptIn) {
      await initAA({});
      smartType = AA.smartAccountType || (AA.sponsored ? 'aa4337' : 'fallback');
    }
  } catch {}
...
async function renderButtons(state) {
  ...
  const makeButton = (label) => {
    ...
    b.style.cssText = [
      ...
      'background:#2d6aee','color:#fff','font-weight:600','cursor:pointer'
    ].join(';');
    b.textContent = label;
    b.addEventListener('mouseenter', () => { if (!b.disabled) b.style.background = '#2f7bff'; });
    b.addEventListener('mouseleave', () => { b.style.background = '#2d6aee'; });
    return b;
  };
  ...
  const safeRun = (btn, fn, labelBusy) => {
    btn.addEventListener('click', async () => {
      ...
      try { await fn(); }
      catch (e) { console.error('[aa/ui] action failed', e); alert(e?.message || 'Operation failed.'); }
      finally { setBusy(btn, false); hydrate().catch(() => {}); }
    });
  };
  ...
  if (!chainOk) {
    ...
    safeRun(switchBtn, async () => {
      ...
      if (!ok) throw new Error('Wallet did not switch to Monad.');
    }, 'Switching...');
  ...
  // Envio Activity pill (one-shot)
  // Activity pill removed per request; activity details live in wallet modal only.
}
