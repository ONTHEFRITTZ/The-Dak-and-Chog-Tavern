...
// ENV: PORT (default 3000) GAME_TYPES (comma-separated: FARO,POKER; default FARO) ADMIN_ADDR RT_RAKE_BPS
...
function ensureStats(addr){ const key=(addr||'').toLowerCase(); if(!stats.has(key)) stats.set(key,{rounds:0,wagered:0,won:0,lost:0}); return stats.get(key); }

/* ------------------------------- Audit logging ----------------------------- */
function logFileName(){
  const d = new Date(); const y=d.getUTCFullYear(); const m=String(d.getUTCMonth()+1).padStart(2,'0'); const day=String(d.getUTCDate()).padStart(2,'0');
  return path.join(LOGS_DIR, `audit-${y}${m}${day}.log`);
}
function audit(tableId, type, payload){ const line = JSON.stringify({ ts:new Date().toISOString(), tableId, type, payload }) + '\n'; fs.appendFile(logFileName(), line, ()=>{}); }
...
function allow(socketId, event){ const cfg=RLIMIT[event]; if(!cfg) return true; const now=Date.now(); if(!buckets.has(socketId)) buckets.set(socketId,{}); const slot=buckets.get(socketId); if(!slot[event]) slot[event]=[]; const arr=slot[event].filter(t=>now-t<cfg.windowMs); arr.push(now); slot[event]=arr; return arr.length<=cfg.limit; }
...
function nextIdFor(prefix){
  ...
  const next = nums.length ? Math.max(...nums)+1 : 1; return `${prefix}${next}`;
}
...
function ensurePokerTable(cat){ const existing=pokerTablesBy(cat); if(existing.length===0){ const id=nextIdFor(idPrefixFor(cat)); return getTable(id);} return existing[0]; }
...
function tablePublic(t){
  ...
  return {
    ...
    seats: t.seats.map(s => s && ({
      ...
      balance: Number(s.balance || 0), lastActive: Number(s.lastActive || 0),
      x: (publicProfiles.get(s.addr || '') || {}).x || null,
      chips: Number(s.chips || 0)  // <— expose chips to client
    })),
    started: !!t.started,
...
function seatFirstEmpty(t, addr, socketId='bot'){
  ...
  if (slot >= 0) {
    ...
    if (t.category === CAT.OFFCHAIN_NL && !Number.isFinite(t.seats[slot].chips)) t.seats[slot].chips = 100; // bot gets chips too
    return slot;
  }
  return -1;
}
function reconcileBotPolicy(t){
  ...
  if (humans === 1 && botIdx === -1) {
    seatFirstEmpty(t, 'bot:dev', 'bot'); t.devBotEnabled = true;
...
function pruneEmpties(cat){
  ...
  const now=nowMs();
  list.forEach(t => { const sc=seatCount(t); if(sc===0){ if(!t.emptySince) t.emptySince=now; } else { t.emptySince=null; } });
  ...
  if (wouldRemain >= minKeep) { tables.delete(candidate.id); audit(candidate.id, 'prune', { category: candidate.category }); }
}
function ensureLobbyPolicy(){
  if (gameEnabled('FARO')) {
    ...
    if (empties.length >= 2) {
      const c = empties[0]; if (!c.emptySince) c.emptySince = now;
      if ((now - c.emptySince) > EMPTY_PRUNE_MS && faro.length > 1) { tables.delete(c.id); audit(c.id, 'prune', { kind: 'FARO' }); }
    }
  }
...
function bestFiveUsed(hole, board){
  try {
    ...
    for (const pick of comb5(idxs)){ const cards = pick.map(i=> all[i]); const r=evaluate7(cards); if(!best || cmpRank(r,best)>0){ best=r; bestPick=pick; } }
    const usedHole=[], usedCommunity=[]; (bestPick||[]).forEach(i=>{ if(i<2) usedHole.push(i); else usedCommunity.push(i-2); });
...
function scheduleTurnTimer(tableId, t){
  ...
  t.poker.turnTimer=setTimeout(()=>{ try{
    const state=t.poker; if(!state) return; const actor=state.actors[state.turnIndex]; if(!actor) return;
...
function startPokerHand(tableId, t){
  try{
    ...
    const community = []; const pot = 0;
    const dealerIndex = 0;
    const sbIndex = (dealerIndex + 1) % actors.length;
    const bbIndex = (dealerIndex + 2) % actors.length;

    // ---- OFFCHAIN stacks + blinds ----
    const SB = 1, BB = 2;
    actors.forEach(a => {
      const seat = t.seats[a.seatId];
      if (t.category === CAT.OFFCHAIN_NL) {
        const base = Number(seat?.chips || 0);
        a.stack = Number.isFinite(base) && base > 0 ? base : 100;   // default 100
      } else {
        a.stack = Number(seat?.chips || 0); // may be undefined
      }
    });
    let toCall = BB;
    let newPot = pot;
    function post(amount, actor) {
      if (t.category !== CAT.OFFCHAIN_NL) { actor.contrib = amount; newPot += amount; return; }
      const pay = Math.min(amount, Math.max(0, Number(actor.stack || 0)));
      actor.contrib = pay;
      actor.stack = Math.max(0, Number(actor.stack || 0) - pay);
      newPot += pay;
    }
    post(SB, actors[sbIndex]);
    post(BB, actors[bbIndex]);
    ...
    t.poker = { stage:'preflop', deck, community, actors, dealerIndex, sbIndex, bbIndex, dealerSeatId, pot:newPot, toCall, turnIndex, startedAt:nowMs(), rng:{ commit, seed } };
    audit(tableId, 'handStart', { commit, seated: actors.map(a=>a.addr), dealerSeatId, sbIndex, bbIndex });
    emitPokerState(tableId, t); sendAllPrivateHoles(t); scheduleTurnTimer(tableId, t); maybeTriggerBot(tableId, t);
  }catch{}
}
...
function emitPokerState(tableId, t){
  try{
    ...
    const pubActors=state.actors.map((a,i)=>({ addr:a.addr, seatId:a.seatId, folded:!!a.folded, acted:!!a.acted, contrib:Number(a.contrib||0), isDealer:(i===state.dealerIndex), isSB:(i===state.sbIndex), isBB:(i===state.bbIndex) }));
    io.to(tableId).emit('poker:state', { stage:state.stage, pot:Number(state.pot||0), toCall:Number(state.toCall||0), community:Array.from(state.community||[]), turnIndex:state.turnIndex, turnAddr:state.actors?.[state.turnIndex]?.addr||null, dealerIndex:state.dealerIndex, actors:pubActors, table: tablePublic(t), rngCommit: state.rng?.commit||null });
  }catch{}
}
...
function advancePokerStage(tableId, t){
  try{
    ...
    if (state.stage === 'preflop') { state.community.push(state.deck.pop(), state.deck.pop(), state.deck.pop()); state.stage='flop'; }
      ...
      if (alive.length > 0) {
        ...
        if (evals.length) {
          ...
          for (let i=1;i<evals.length;i++){ const cmp=cmpRank(evals[i].ev,best); if(cmp>0){ best=evals[i].ev; bestIdxs=[evals[i].idx]; } else if (cmp===0){ bestIdxs.push(evals[i].idx); } }
          ...
          try {
            // write stacks back to seats for OFFCHAIN_NL
            if (t.category === CAT.OFFCHAIN_NL) {
              actors.forEach(a => { const seat=t.seats[a.seatId]; if (seat && typeof a.stack==='number') seat.chips = Number(a.stack||0); });
            }
          } catch {}
        }
      }
      const exposures = actors.filter(a=>!a.folded).map(a => ({ addr:a.addr, cards:Array.from(a.cards||[]) }));
      io.to(tableId).emit('poker:hand', { winners, community:board, exposures, pot:state.pot||0, rng:{ commit:state.rng?.commit, seed:state.rng?.seed }, table: tablePublic(t) });
    ...
    let idx=(state.dealerIndex+1)%actors.length, spins=0; while(actors[idx]?.folded && spins<actors.length){ idx=(idx+1)%actors.length; spins++; }
    state.turnIndex=idx;
    audit(tableId,'stage',{ stage:state.stage, community:Array.from(state.community||[]) });
    emitPokerState(tableId,t); scheduleTurnTimer(tableId,t); maybeTriggerBot(tableId,t);
  }catch{}
}

/* ----------------------------- Actions (authoritative) --------------------- */
function bettingRoundComplete(state){ const target=Number(state.toCall||0); return state.actors.filter(a=>!a.folded).every(a=> a.acted && Number(a.contrib||0)===target); }

function applyAction(tableId, t, addrLower, action, isAuto=false){
  try{
    if (!t?.poker) return;
    const state=t.poker; const idx=state.turnIndex; const actors=state.actors;
    if (idx<0 || idx>=actors.length) return;
    const actor=actors[idx]; if (!actor || actor.addr!==addrLower) return;
    ...
    if (action === 'fold') {
      ...
      if (alive.length === 1) {
        io.to(tableId).emit('poker:hand', { winners:[{addr:alive[0].addr}], community:Array.from(state.community||[]), pot:state.pot||0, rng:{ commit:state.rng?.commit, seed:state.rng?.seed }, table: tablePublic(t) });
        try { state.actors.forEach(a => clearPrivateHoleForSeat(t, a.seatId)); } catch {}
      ...
      if (need <= 0) {
        ...
        // treat as call when "check" needs chips
        let pay = need;
        if (t.category === CAT.OFFCHAIN_NL) {
          pay = Math.min(need, Math.max(0, Number(actor.stack || 0)));
          actor.stack = Math.max(0, Number(actor.stack || 0) - pay);
        }
        actor.contrib = Number(actor.contrib || 0) + pay;
        state.pot = Number(state.pot || 0) + pay;
        actor.acted = true;
      }
    } else if (action === 'call') {
      const need = Math.max(0, Number(state.toCall||0) - Number(actor.contrib||0));
      ...
      actor.acted = true; // ignore raises/bets in this beta
    }

    // next active
    let next=(state.turnIndex+1)%actors.length, loop=0; while(actors[next] && actors[next].folded && loop<actors.length){ next=(next+1)%actors.length; loop++; }
    state.turnIndex=next;

    if (bettingRoundComplete(state)) { audit(tableId,'roundComplete',{ stage:state.stage, pot:state.pot }); advancePokerStage(tableId,t); maybeTriggerBot(tableId,t); return; }
    emitPokerState(tableId,t); scheduleTurnTimer(tableId,t); maybeTriggerBot(tableId,t);
  }catch{}
}

/* ------------------------------ Dev helper bot ----------------------------- */
function nextActiveIndex(actors, from){ if(!actors.length) return -1; let i=(from+1)%actors.length,spins=0; while(spins<actors.length){ if(!actors[i]?.folded) return i; i=(i+1)%actors.length; spins++; } return -1; }
...
io.on('connection', (socket) => {
  ...
  socket.on('join_table', (m) => {
    try {
      ...
      // refresh private cards on rejoin
      if (isPoker(t) && t.poker && addrLower) {
        ...
        if (seatIdx >= 0) {
          ...
          if (actor && actor.cards) { try { t.seats[seatIdx].socketId = socket.id; } catch {} sendPrivateHoleToSeat(t, seatIdx, actor.cards); }
        }
    ...
    try {
      ...
      if (t.kind === 'FARO') {
        if (allReady && t.bets.size > 0) {
          const bankRank = rand13(); const playerRank = rand13();
      ...
      if (t.category === CAT.OFFCHAIN_NL && t.devBotEnabled) {
        try { const botIdx=findBotIndex(t); if (botIdx >= 0) t.seats[botIdx].ready = true; } catch {}
      }
  ...
  socket.on('poker:act', (m) => {
    try {
      ...
      const state=t.poker; const actorIdx=state.turnIndex; const actors=state.actors;
      if (actorIdx<0 || actorIdx>=actors.length) return; const actor=actors[actorIdx]; if(!actor) return;
  ...
  socket.on('sim:rebuy', () => {
    try {
      ...
      const sIdx = t.seats.findIndex(s => s && s.addr === addrLower);
      if (sIdx < 0) return;
      const seat = t.seats[sIdx];
      seat.chips = Number(seat.chips || 0) + 100;
      t.lastActive = nowMs();
      emitUpdate(t);
    } catch {}
  });
...
(function restoreState(){
  try {
    ...
    arr.forEach(o => {
      ...
      t.poker=null; if (isPoker(t)) reconcileBotPolicy(t);
    });
    emitLobby();
  } catch {}
})();
