...
async function buildAA4337Account(injected, { bundlerUrl, paymasterUrl }) {
  ...
  async function sendViaZeroDevUO(tx){
    try {
      ...
      const decodePaymasterError = (payloadError) => {
        if (!payloadError || typeof payloadError !== 'object') return payloadError;
        const detail = {
          message: payloadError.message || '',
          code: payloadError.code
        };
        const tryDecodeHex = (hex) => {
          if (!hex || typeof hex !== 'string' || !hex.startsWith('0x')) return null;
          try { return ethers.utils.toUtf8String(ethers.utils.arrayify(hex)); } catch { return null; }
        };
        const cause = (payloadError.data && payloadError.data.cause) || payloadError.cause || null;
        if (cause?.message && !detail.causeMessage) detail.causeMessage = cause.message;
        const candidates = [];
        if (Array.isArray(payloadError.data?.errors)) {
          for (const item of payloadError.data.errors) {
            if (item?.data) candidates.push(item.data);
          }
        }
        candidates.push(
          payloadError.data?.cause?.revert,
          payloadError.data?.cause?.data,
          payloadError.data?.revert,
          payloadError.data?.data,
          payloadError.data,
          cause?.revert,
          cause?.data
        );
        for (const candidate of candidates) {
          const msg = tryDecodeHex(candidate);
          if (msg) {
            detail.revert = msg;
            break;
          }
      ...
      try {
        if (paymasterUrlEffective) {
          const callPaymasterRpc = async (method, params) => {
            ...
            if (payload.error) {
              try {
                const decoded = decodePaymasterError(payload.error);
                console.warn('[aaClient] paymaster error payload', decoded, { endpoint: paymasterUrlEffective });
                if (decoded?.revert && typeof decoded.revert === 'string') {
                  if (decoded.revert.includes('VerifyingPaymaster')) {
                    setStatus('Gas sponsor rejected: paymaster signature missing or invalid.', 'error');
                  } else if (decoded.revert.includes('AA23')) {
                    setStatus('Gas sponsor rejected by paymaster (AA23).', 'error');
                  }
                }
              } catch {
                console.warn('[aaClient] paymaster error payload', payload.error, { endpoint: paymasterUrlEffective });
              }
      ...
      if (bundlerClient && typeof bundlerClient.sendUserOperation === 'function') {
        try {
          ...
          let txHash = null;
          try {
            if (typeof bundlerClient.waitForUserOperationReceipt === 'function') {
